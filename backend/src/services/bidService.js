 ...
 * All bid logic is server-authoritative
 */
const processBid = async ({ auctionId, auctionItemId, teamId, bidderId, amount, ipAddress }) => {
  ...
  try {
    ...
    const itemResult = await client.query(
      `SELECT ai.*, a.bid_increment, a.bid_cap_enabled, a.bid_cap_amount,
              a.anti_sniping_enabled, a.anti_sniping_trigger_window, a.anti_sniping_extension, a.anti_sniping_max_extensions,
    ...
    if (item.auction_status !== 'live' && item.auction_status !== 'round2_live' && item.auction_status !== 'round3_live') {
      await client.query('ROLLBACK');
    ...
    // 7. Cannot bid on own current leader
    ...
    const minBid = item.current_price
      ? item.current_price + item.bid_increment
      : item.current_price || item.bid_increment;
    ...
    // 9. Check bid cap
    if (item.bid_cap_enabled && amount > item.bid_cap_amount) {
      await client.query('ROLLBACK');
    ...
    await client.query(
      `UPDATE auction_items
       SET current_price = $1, current_leader_team_id = $2, updated_at = NOW()
       WHERE id = $3`,
    ...
    const capReached = item.bid_cap_enabled && amount >= item.bid_cap_amount;
    ...
    await auditService.log({
      ...
      entityType: 'bid',
      entityId: bid.id,
      details: { amount, teamId, playerId: item.player_id, capReached },
      ipAddress,
    });
...
const submitTieBreakBid = async ({ tieBreakRoundId, teamId, bidderId, amount }) => {
  ...
  try {
    ...
    // Verify tie-break round is open
    ...
    const teamResult = await client.query(
      `SELECT remaining_budget FROM teams WHERE id = $1`,
      [teamId]
    );
    ...
    if (amount < (round.bid_cap_amount || 0)) {
      await client.query('ROLLBACK');
 ...
 * Returns winner or opens next tie-break round if still tied
 */
const resolveTieBreakRound = async (tieBreakRoundId, auctionItemId, io) => {
  ...
  try {
    ...
    // Get tie-break config
    const roundInfoResult = await client.query(
      `SELECT tbr.round_number, a.max_tie_break_rounds, a.tie_break_mode, ai.auction_id, a.id AS auction_id_val
       FROM tie_break_rounds tbr
       JOIN auction_items ai ON tbr.auction_item_id = ai.id
       JOIN auctions a ON ai.auction_id = a.id
       WHERE tbr.id = $1`,
    ...
    if (topBids.length === 1) {
      ...
      await client.query(
        `UPDATE tie_break_rounds
         SET status = 'resolved', winner_team_id = $1, winning_amount = $2, closed_at = NOW()
         WHERE id = $3`,
        [winner.team_id, topAmount, tieBreakRoundId]
      );

      // Sell player to winner
      await client.query(
    ...
    // Still tied - check if we can do another round
    const canContinue = maxRounds === 0 || currentRound < maxRounds;
    if (!canContinue) {
      // Use configured fallback (lucky draw or organizer decision)
      await client.query(
        `UPDATE tie_break_rounds SET status = 'closed', closed_at = NOW() WHERE id = $1`,
        [tieBreakRoundId]
      );
      await client.query('COMMIT');
